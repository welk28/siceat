...
class Docente extends CI_Controller {
	...
	public function __construct(){
		...
		$this->load->model("su/Docente_model_su");
		$this->load->model("Altatrim_model");
	}
	public function index()
	{
		$data  = array(
			'docentes' => $this->Docente_model_su->getDocente(), 
		);
		$this->load->view("layouts/header");
		$this->load->view("su/docente",$data);
	...
	public function materias(){
		$data = array(
			'docentes' =>  $this->Docente_model_su->getDocencurso()
		);
		$this->load->view("layouts/header");
		$this->load->view("su/docmaterias",$data);
		$this->load->view("layouts/footer");
	}
	...
	public function califica(){
		...
		if($trim==1){
			$resp= $this->Docente_model->califica1($idh, $matricula, $tr1 );
		}else{
			if($trim==2){
				$resp= $this->Docente_model->califica2($idh, $matricula, $tr2 );
			}else{
				if($trim==3){
					$resp= $this->Docente_model->califica3($idh, $matricula, $tr3 );
				}else{
					if($trim==4){
						$resp= $this->Docente_model->califica1($idh, $matricula, $tr1 );
						$resp= $this->Docente_model->califica2($idh, $matricula, $tr2 );
						$resp= $this->Docente_model->califica1($idh, $matricula, $tr3 );
					}
				}
			}
		}

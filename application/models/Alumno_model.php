...
class Alumno_model extends CI_Model {
	...
	public function getTodos(){
		...
		$resultado=$this->db->query("select distinct a.matricula, a.nom, a.app, a.apm, a.sexo, a.fecnac, a.grado, a.grupo, a.idt, t.nomt
									...
									where t.idt=a.idt and a.turno='$turno' order by a.grado, a.grupo, a.app, a.apm, a.nom;");
	...
	public function materiasBoleta($matricula){
		...
		$resultado=$this->db->query("select c.matricula, c.idh, m.nommat, c.tr1, c.tr2, c.tr3, c.prom, c.fa1, c.fa2, c.fa3  from materia m, cursa c, horario h where m.idmat=h.idmat and h.idh=c.idh and h.idp='$periodo' and matricula='$matricula';");
	...
	public function Horalumno($matricula){
		$periodo= $this->session->userdata("periodo");
		$turno=$this->session->userdata("turno");
		$resultados=$this->db->query("select g.idg, g.grado, g.grupo, h.idh, m.nommat, p.nomp from grupo g, personal p, materia m, cursa c, horario h where g.idg=h.idg and h.rfcp=p.rfcp and m.idmat=h.idmat and h.idh=c.idh and h.idp='$periodo' and c.matricula='$matricula';");
		$resultados->result();
  ...
  public function getAlumnosI($grado, $grupo){
	...
	public function getAlumnosT($idmat, $grado){
		$periodo= $this->session->userdata("periodo");
		$turno=$this->session->userdata("turno");
	...
	public function verCal($idh, $matricula){
		$resultado=$this->db->query("select tr1, tr2, tr3 from cursa where matricula='$matricula' and idh=$idh");
		return $resultado->row();
	}
	...
	public function TallerActual($grado, $idt){
		$periodo= $this->session->userdata("periodo");
		$turno=$this->session->userdata("turno");
		$resultado=$this->db->query("SELECT * FROM horario h, grupo g WHERE h.idg=g.idg and h.idp='$periodo' AND h.turno='$turno' AND h.idmat='$idt' AND g.grado=$grado");
		return $resultado->row();
	}
	public function verifInscr($matricula){
		$periodo= $this->session->userdata("periodo");
		$turno=$this->session->userdata("turno");
		$resultado=$this->db->query("select distinct c.matricula from cursa c, horario h where c.idh=h.idh and h.idp='$periodo' AND h.turno='$turno' and c.matricula='$matricula'");

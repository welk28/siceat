...
class Docente_model extends CI_Model {
	public function getDocente(){
		...
		$resultados=$this->db->query("select * from personal where turno='$turno' order by nomp");
	...
	public function getDocencurso(){
		...
		$resultados=$this->db->query("select distinct h.rfcp, p.nomp 
								...
								where p.rfcp=h.rfcp and h.turno=p.turno and  h.turno='$turno' and h.idp='$periodo';");
		if ($resultados->num_rows() > 0) {
			return $resultados->result();
		}
		else{
	...
	public function docente($rfcp){
		$resultados=$this->db->query("select * from personal where rfcp='$rfcp';");
		if ($resultados->num_rows() > 0) {
	...
	public function docentemat($idh){
		$periodo= $this->session->userdata("periodo");
		$resultados=$this->db->query("SELECT h.idh, h.idmat, h.idg, h.turno, h.rfcp, g.idg, p.nomp, g.grado, g.grupo, m.nommat from horario h, personal p, grupo g, materia m where m.idmat=h.idmat and g.idg=h.idg and p.rfcp=h.rfcp and idh='$idh';");
		if ($resultados->num_rows() > 0) {
			return $resultados->row();
		}
		else{
	...
	public function califica1($idh, $matricula, $tr1, $fa1, $prom){
		$fecha=date("Y-m-d");
		$modifica=$this->session->userdata("personal");
		$resultado=$this->db->query("update cursa set tr1=$tr1, ftr1='$fecha', fa1=$fa1, asigna1='$modifica', prom=$prom where idh=$idh and matricula='$matricula'");
		if($this->db->affected_rows()>0){
			return true;
		}else{
			return false;
	...
	public function califica2($idh, $matricula, $tr2, $fa2, $prom){
		$fecha=date("Y-m-d");
		$modifica=$this->session->userdata("personal");
		$resultado=$this->db->query("update cursa set tr2=$tr2, ftr2='$fecha', fa2=$fa2, asigna2='$modifica', prom=$prom where idh=$idh and matricula='$matricula'");
		if($this->db->affected_rows()>0){
			return true;
		}else{
			return false;
	...
	public function califica3($idh, $matricula, $tr3, $fa3, $prom){
		$fecha=date("Y-m-d");
		$modifica=$this->session->userdata("personal");
		$resultado=$this->db->query("update cursa set tr3=$tr3, ftr3='$fecha', fa3=$fa3, asigna3='$modifica', prom=$prom where idh=$idh and matricula='$matricula'");
		if($this->db->affected_rows()>0){
			return true;
		}else{
			return false;
	...
	public function matTaller($idh){
		$resultados=$this->db->query("select * from horario h, taller t WHERE h.idmat=t.idt AND h.idh=$idh");
		if ($resultados->num_rows() > 0) {
			return 1;
		}
		else{

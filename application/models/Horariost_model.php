...
class horariost_model extends CI_Model {
	public function getHorarios(){
		$periodo= $this->session->userdata("periodo");
		$turno=$this->session->userdata("turno");
		$resultados=$this->db->query("SELECT h.idh, h.idg, g.grado, g.grupo, m.idmat, m.nommat FROM grupo g, horario h, taller t, materia m WHERE g.idg=h.idg and h.idmat=m.idmat and t.idt=m.idmat AND h.idp='$periodo' AND h.turno='$turno' order by h.idh");
		$resultados->result();

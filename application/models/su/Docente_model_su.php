...
class Docente_model_su extends CI_Model {
	...
	public function getDocencurso(){
		...
		$resultados=$this->db->query("select distinct h.rfcp, p.nomp, p.turno 
								from horario h, personal p 
								where p.rfcp=h.rfcp  and  h.turno='$turno' and h.idp='$periodo';");
		if ($resultados->num_rows() > 0) {
			return $resultados->result();
		}
		else{
	...
	public function matedoc($rfcp){
		...
		$resultados=$this->db->query(" select * from horario h, grupo g, materia m where m.idmat=h.idmat and g.idg=h.idg and h.rfcp='$rfcp' and idp='$periodo' ");
		if ($resultados->num_rows() > 0) {
			return $resultados->result();
		}
		else{
